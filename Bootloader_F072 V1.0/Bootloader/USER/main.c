...
void GetDeviceInfo(void)
{
	...
	DeviceInfoBuffer[0] = (u8)121;  //软件版本 11表示V1.1
	for(i=1;i<=12;i++)
...
//	usbsendData(TransportProtocol_Manager.Buf, TransportProtocol_Manager.FrameTotalLength);	
    ...
    {
        USART_SendData(USART1,TransportProtocol_Manager.Buf[i]);
...
int main()
{
    ...
    {
        //初始化传输协议  指向串口接收缓冲区  选择sum的校验方式
        TransportProtocol_Init(&TransportProtocol, usart_fifo.buffer, Checksum_Sum);
    }
	while(1) 
	{	
        
        
		if(uu_rxfifo.writeptr && usbstatus == CONFIGURED)
		{
				...
				{	
					...
					{	
						...
						{	
                            ...
                            TransportProtocol.Data_Length = 1;            //有效数据大小
                            TransportProtocol.Data = &IsTransportOK;			     	      //要发送的数据       
        ...
        else if(usart_fifo.writeptr &&usbstatus !=CONFIGURED)
        {
            if(oldcount == usart_fifo.writeptr)//新周期内,没有收到任何数据,认为本次数据接收完成.
			{
				//获取接收的总字节数
				TransportProtocol_Manager.RecieveByteCount = usart_fifo.writeptr;
				//解包
				res = TransportProtocol_Manager.Unpacked();
				if(res!=UPACKED_SUCCESS)  //解包失败的时候，不响应上位机，上位机会自动重发
				{	
				...
				{	
					...
					USART_IAP_Response();  //响应上位机						
				}	
                
                usart_fifo.writeptr = 0;
            }
            else 
            {
                oldcount = usart_fifo.writeptr;	
            }
            timeOut = 0;	//超时清零 
        }else
		{
			delay_ms(50);
			if(IsTransportOK == 1 || timeOut++ > 2000)	/*60S 超时退出bootloader*/
			{
				timeOut = 0;
				iap_load_app(FIRMWARE_START_ADDR);
			}			

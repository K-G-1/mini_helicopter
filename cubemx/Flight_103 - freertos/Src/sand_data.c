...
extern osThreadId SandTaskHandle;
extern osSemaphoreId USART_SamHandle;
/*****************************************************************/
...
/************************************接收数据并处理******************************************************/
extern osMutexId USARTMutexHandle;
static void Send_Check(uint8_t head,uint8_t check_sum)
{
  ...
  if(osMutexWait(USARTMutexHandle,0xff) == osOK)
    HAL_UART_Transmit(&huart1,Tx_buff,7,0xff);
  osMutexRelease(USARTMutexHandle);
}
uint8_t RxBuffer[50],_data_cnt;
void Data_Receive_Prepare(uint8_t data)
{
	...
	else if(state==5)
	{
		state = 0;
		RxBuffer[4+_data_cnt]=data;
    osSemaphoreRelease(USART_SamHandle);
//    osThreadSuspendAll();  

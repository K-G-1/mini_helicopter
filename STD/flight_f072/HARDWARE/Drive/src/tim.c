...
void TIM_Init(void)
{
	...
	TIM_TimeBaseInitStruct.TIM_Period=48-1;   //设置自动重装载的周期值
	TIM_TimeBaseInitStruct.TIM_Prescaler=1000-1;   //设置预分频值
	TIM_TimeBaseInitStruct.TIM_ClockDivision=0;   //设置时钟分割
	TIM_TimeBaseInitStruct.TIM_CounterMode=TIM_CounterMode_Up;   //向上计数模式
	TIM_TimeBaseInit(TIM2,&TIM_TimeBaseInitStruct);   //定时器初始化函数
	
	TIM_ITConfig(TIM2,TIM_IT_Update,ENABLE);   //TIM4中断使能
	
	TIM_Cmd(TIM2,ENABLE);   //TIM4使能
}
